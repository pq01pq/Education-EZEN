...
class MyCalendar06 extends Frame implements ActionListener {
	...
	private Button lastYear, lastMonth, nextMonth, nextYear, planSaveButton;
	...
	public void actionPerformed(ActionEvent e) {
		...
		case "arrow":
			Button arrow = (Button)e.getSource();
			if(arrow.getLabel().equals(lastYear.getLabel())) {
				calendar.set(
						calendar.get(Calendar.YEAR) - 1, calendar.get(Calendar.MONTH), 1);
			} else if(arrow.getLabel().equals(lastMonth.getLabel())) {
				calendar.set(
						calendar.get(Calendar.YEAR), calendar.get(Calendar.MONTH) - 1, 1);
			} else if(arrow.getLabel().equals(nextMonth.getLabel())) {
				calendar.set(
						calendar.get(Calendar.YEAR), calendar.get(Calendar.MONTH) + 1, 1);
			} else if(arrow.getLabel().equals(nextYear.getLabel())) {
				calendar.set(
						calendar.get(Calendar.YEAR) + 1, calendar.get(Calendar.MONTH), 1);
			} 
			setYearMonth();
			...
			break;
		case "day":
			Button loadDayButton = (Button)e.getSource();
			File loadFile = new File("dir" + File.separator + calendar.get(Calendar.YEAR) + "_" +
					(calendar.get(Calendar.MONTH) + 1) + "_" + loadDayButton.getLabel() + ".txt");
			String text = null;
			DataInputStream dis;
			try {
				loadFile.createNewFile();
				dis = new DataInputStream(new BufferedInputStream(new FileInputStream(loadFile)));
				this.lookingDay = Integer.parseInt(loadDayButton.getLabel());
				planDialog.setTitle(calendar.get(Calendar.YEAR) + "년 " +
						(calendar.get(Calendar.MONTH) + 1) + "월 " + lookingDay + " 일 계획");
				text = dis.readUTF();
...
//			Button saveDayButton = (Button)e.getSource();
			File saveFile = new File("dir" + File.separator + calendar.get(Calendar.YEAR) + "_" +
					(calendar.get(Calendar.MONTH) + 1) + "_" + lookingDay + ".txt");
			try {
				DataOutputStream dos = new DataOutputStream(new BufferedOutputStream(new FileOutputStream(saveFile)));
				dos.writeUTF(planArea.getText());
				dos.close();
	...
	public void init() {
		...
		planDialog = new Dialog(this, "", false);
		planDialog.setSize(500, 300);
		planDialog.setLocation(this.getX(), this.getY());
		planDialog.setLayout(new BorderLayout());
		planDialog.setVisible(false);
		
		planArea = new TextArea();
		planArea.setFont(new Font("", Font.PLAIN, 20));
		planDialog.add("Center", planArea);
		
		planSaveButton = new Button("저장");
		planSaveButton.setPreferredSize(new Dimension(50, 30));
		planSaveButton.addActionListener(this);
		planSaveButton.setActionCommand("save");
		planDialog.add("South", planSaveButton);
	}
	...
	public void setArrows() {
		lastYear.setLabel("◀◀");
		lastYear.setActionCommand("arrow");
		lastMonth.setLabel(" ◀ ");
		lastMonth.setActionCommand("arrow");
		
		nextMonth.setLabel(" ▶ ");
		nextMonth.setActionCommand("arrow");
		nextYear.setLabel("▶▶");
		nextYear.setActionCommand("arrow");
	...
	public void setDayButtons() {
		...
		while(day <= finalDay) {
			...
			dayButton[indexOfDayGrid].addActionListener(this);
			dayButton[indexOfDayGrid].setActionCommand("day");
